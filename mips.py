...
#
    ...
    elif operation == 'udiv': arm_code += "\tudiv " + regi1 + ", " + regi1 + ", " + regi2 + "\n"
    ...
    while(i < len(inter)):
        ...
        if len(parts) == 1:
            ...
            # m#[#] = t#
                ...
                if len(next_inter_line) > 3:
                    if  next_inter_line[3] == "out_string" or next_inter_line[3] == "in_string" or next_inter_line[3] == "in_int" or next_inter_line[3] == "out_int":
                        ...
                        arm_code += "\tmov " + reg1 + ", #" + "1" + "\n"
                        arm_code += "\tmov " + reg2 + ", " + "=message" + str(conto) + "\n"
                        arm_code += "\tmov " + reg3 + ", #" + str(next_next_inter_line[1])  + "\n"
                        arm_code += "\tmov " + reg4 + ", #" + "4"  + "\n"
                        arm_code += "\tswi " + "0"  + "\n"
                        arm_code += "\tmov " + reg4 + ", #" + "1"  + "\n"
                        arm_code += "\tswi " + "0"  + "\n"
            ...
            # t# = ~m#[#]
                ...
                reg1 = REGISTERS.pop()
                arm_code += "\tmov " + reg1 + ", " + "=text" + str(contoText) + "\n"
                text_consola += "text" + str(contoText) + ":" + "\n"
                text_consola += "\t.ascii " + str(parts[2])  + "\n"
                contoText += 1
                REGISTERS.append(reg1)
            # m#[#] = m#[#]
            else:
                ...
                if param[0] == 'param':
                    if 't' in param[1]:
                        ...
                        if len(para) >= 3:
                            if para[2] == 'allocate':
                                reg1 = REGISTERS.pop()
                                arm_code += "\tldr " + str(reg1) + ", =" + str(para[4])  + "\n"
                                if push_param[0] == "push":
                                    if  "[" in push_param[2]:
                                        reg2 = REGISTERS.pop()
                                        memory_address = getBracketsContent(push_param[2])
                                        arm_code += "\tstr " + reg2 + ", [sp, #" + str(memory_address) + "]\n"
                                        REGISTERS.append(reg2)                            
                                        REGISTERS.append(reg1)

                                else:
                                    REGISTERS.append(reg1)
                arm_code += "\tbl " + str(parts[3])  + "\n"
                # i += 1
                # continue
            ...
            if parts[3] == "+":
                ...
                arm_code += operationsFunction(first_operand,second_operand,'udiv',inter,i)
            # IfZ
